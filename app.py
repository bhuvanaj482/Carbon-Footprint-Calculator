...
# Initialize page config with a default icon first
st.set_page_config(layout="wide", page_title="Carbon Footprint Calculator")

def get_base64(bin_file):
    try:
        abs_path = get_abs_path(bin_file)
        ...
        st.error(f"Error loading file {bin_file}: {str(e)}")
        return ""

# Create required directories if they don't exist
os.makedirs(get_abs_path("media"), exist_ok=True)
os.makedirs(get_abs_path("style"), exist_ok=True)
os.makedirs(get_abs_path("models"), exist_ok=True)

# Update media file paths
background = get_base64("media/CLIMATE.jpg")
icon2 = get_base64("media/icon2.png")
icon3 = get_base64("media/icon3.png")

# Update style file paths
with open(get_abs_path("style/style.css"), "r") as style:
    css=f"""<style>{style.read().format(background=background, icon2=icon2, icon3=icon3)}</style>"""
    st.markdown(css, unsafe_allow_html=True)

def script():
    with open(get_abs_path("style/scripts.js"), "r", encoding="utf-8") as scripts:
...
# Update model paths
ss = pickle.load(open(get_abs_path("models/scale.sav"), "rb"))
model = pickle.load(open(get_abs_path("models/model.sav"), "rb"))
...
# Update footer path
with open(get_abs_path("style/footer.html"), "r", encoding="utf-8") as footer:
